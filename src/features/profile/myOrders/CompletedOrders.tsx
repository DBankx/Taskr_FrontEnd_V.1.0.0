...
const CompletedOrders = () => {
    const {getAllOrders, loadingOrders, completedOrders} = useContext(rootStoreContext).orderStore;
    useEffect(() => {
        getAllOrders("COMPLETED");
    }, [getAllOrders])
    if(loadingOrders || completedOrders === null) return <Loader />
    return (
        <Box className="task__bid__form__card no__padding">
            <HStack spacing="10px" p="1em">
                <p className="text__darker text__upper text__bold">Completed orders</p>
                <Tooltip hasArrow style={{background: "rgb(41, 43, 50)", fontSize: "0.8em"}} label="These are orders that have been completed sucessfully" aria-label="A tooltip">
...
export default observer(CompletedOrders);